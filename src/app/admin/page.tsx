...
export default async function AdminPage() {
  ...
  return <AppShell email={user.email ?? "관리자"} title="관리자" description="사용자와 게시 이력을 관리합니다.">
    <div className="grid gap-5 md:grid-cols-2"><section className="rounded-xl border border-stone-200 bg-white p-7"><h2 className="font-bold">작성 가이드</h2><p className="mt-2 text-sm text-stone-500">AI 초안에 적용할 품질 기준을 관리합니다.</p><Link href="/settings/guides" className="mt-4 inline-block text-sm font-semibold text-emerald-700 hover:underline">작성 가이드 관리 →</Link></section><section className="rounded-xl border border-dashed border-stone-300 bg-white p-7"><h2 className="font-bold">게시 이력</h2><p className="mt-2 text-sm text-stone-500">아직 게시 이력이 없습니다.</p></section></div>
