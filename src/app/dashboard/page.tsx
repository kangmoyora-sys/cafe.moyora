...
export default async function Dashboard({ searchParams }: { searchParams: Promise<{ notice?: string }> }) {
  ...
  return <AppShell email={user.email ?? "로그인 사용자"} title="콘텐츠 대시보드" description="네이버 카페 게시글 초안을 한곳에서 준비하세요.">
    ...
    <section className="mt-8 rounded-xl border border-dashed border-stone-300 bg-white p-10 text-center"><h2 className="font-bold">최근 콘텐츠</h2><p className="mt-2 text-sm text-stone-500">아직 작성된 콘텐츠가 없습니다. 첫 초안을 만들어 보세요.</p></section>
  </AppShell>;
}
