...
export default async function ContentDetailPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ updated?: string }> }) {
  ...
  const contentSections = makeContentSections(draft.body, draft.images);
  ...
  return <AppShell email={email} title="초안 상세" description="저장한 콘텐츠 초안을 확인합니다.">
    ...
    <article className="max-w-3xl rounded-xl border border-stone-200 bg-white p-6">
      ...
      <section className="mt-6 border-t border-stone-100 pt-6"><div className="flex flex-wrap items-baseline justify-between gap-2"><h3 className="text-sm font-semibold text-stone-500">본문</h3>{draft.images.length > 0 && <p className="text-xs text-stone-500">선택한 이미지는 본문 문단 사이에 배치됩니다.</p>}</div><div className="mt-3 space-y-5">{contentSections.map((section, sectionIndex) => <div key={`${section.paragraph}-${sectionIndex}`} className="space-y-4"><p className="whitespace-pre-wrap leading-7 text-stone-800">{section.paragraph}</p>{section.images.map((image) => <figure key={image.id} className="overflow-hidden rounded-lg border border-stone-200 bg-white"><img src={image.url} alt={image.alt} className="max-h-96 w-full object-cover" /><figcaption className="p-3 text-xs text-stone-600">{image.attribution && image.attributionUrl ? <a href={image.attributionUrl} target="_blank" rel="noreferrer" className="text-emerald-800 hover:underline">{image.attribution}</a> : image.kind === "generated" ? "AI 생성 이미지" : "업로드한 이미지"}</figcaption></figure>)}</div>)}</div></section>
      {draft.status === "approved" ? <PublishingKit title={draft.title} body={draft.body} images={draft.images} /> : <section className="mt-6 rounded-xl bg-stone-100 p-5 text-sm text-stone-700"><p className="font-semibold text-stone-900">발행 준비는 승인 완료 후 사용할 수 있습니다.</p><p className="mt-1 leading-6">내용을 검토한 뒤 상태를 ‘승인 완료’로 바꾸면 카페 또는 블로그용 발행 패키지를 만들 수 있습니다.</p></section>}
    </article>
  </AppShell>;
}
