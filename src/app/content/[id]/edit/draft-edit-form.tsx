...
import type { ContentDraft, DraftStatus } from "@/lib/content-drafts";
...
export function DraftEditForm({ draft, isAdmin }: { draft: ContentDraft; isAdmin: boolean }) {
  ...
  return <form action={formAction} className="max-w-2xl space-y-6 rounded-xl border border-stone-200 bg-white p-6">
    ...
    <label className="block text-sm font-semibold">상태<select name="status" value={values.status} onChange={(event) => setValues({ ...values, status: event.target.value as DraftStatus })} className="mt-2 w-full rounded-lg border border-stone-300 px-3 py-2.5">{statusOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label>
