        ...
        <section className="overflow-hidden rounded-xl border border-stone-200 bg-white">
          <div className="overflow-x-auto">
            <table className="w-full min-w-[640px] text-left text-sm">
              ...
              <tbody>
                {drafts.map((draft) => <tr key={draft.id} className="border-t border-stone-100"><td className="px-5 py-4 font-medium text-stone-900"><Link href={`/content/${draft.id}`} className="hover:text-emerald-700 hover:underline">{draft.title}</Link></td><td className="px-5 py-4 text-stone-600">{draft.user_id === user.id ? "나" : `사용자 ${draft.user_id.slice(0, 8)}`}</td><td className="px-5 py-4"><span className="rounded-full bg-stone-100 px-2.5 py-1 text-xs font-medium text-stone-700">초안</span></td><td className="px-5 py-4 text-stone-600">{formatDraftDate(draft.updated_at)}</td></tr>)}
              </tbody>
            </table>
          </div>
        </section>
