...
import { generateStructuredText, isAIProvider, isAIProviderConfigured, type AIProvider } from "@/lib/ai-provider";
import { readContentImages, type ContentImage } from "@/lib/content-images";
import { createClient, getCurrentUser } from "@/lib/supabase/server";

export type DraftFormState = { error?: string };
export type AIDraftResult = { error?: string; title?: string; body?: string; provider?: AIProvider };
...
export type GeneratedImageResult = { image?: { id: string; dataUrl: string; alt: string }; error?: string };
...
export async function saveDraft(_previousState: DraftFormState, formData: FormData): Promise<DraftFormState> {
  ...
  if (!isAIProvider(aiProvider)) return { error: "유효한 AI 생성 서비스를 선택해 주세요." };
...
export async function generateContentImage(prompt: string): Promise<GeneratedImageResult> {
  ...
  if (process.env.AI_GENERATION_ENABLED !== "true" || !process.env.OPENAI_API_KEY || !process.env.OPENAI_IMAGE_MODEL) return { error: "AI 이미지 생성은 아직 설정되지 않았습니다." };
  const accessError = await requireResearchAccess(user.id);
  ...
  try {
    const response = await fetch("https://api.openai.com/v1/images/generations", {
      ...
      body: JSON.stringify({ model: process.env.OPENAI_IMAGE_MODEL, prompt: imagePrompt, size: "1024x1024", output_format: "png" }),
    });
    if (!response.ok) return { error: "AI 이미지 생성에 실패했습니다. 잠시 후 다시 시도해 주세요." };
    const payload = await response.json() as { data?: Array<{ b64_json?: unknown }> };
    const base64 = payload.data?.[0]?.b64_json;
    if (typeof base64 !== "string" || !base64) return { error: "AI 생성 이미지를 처리하지 못했습니다. 잠시 후 다시 시도해 주세요." };
    return { image: { id: `generated-${crypto.randomUUID()}`, dataUrl: `data:image/png;base64,${base64}`, alt: imagePrompt.slice(0, 300) } };
  } catch {
...
export async function generateAIDraft(formData: FormData): Promise<AIDraftResult> {
  ...
  if (!isAIProvider(providerValue)) return { error: "유효한 AI 생성 서비스를 선택해 주세요." };
  const provider = providerValue;
  if (!isAIProviderConfigured(provider)) return { error: provider === "gemini" ? "Gemini 생성 기능은 아직 설정되지 않았습니다." : "OpenAI 생성 기능은 아직 설정되지 않았습니다." };
